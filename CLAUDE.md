...
- TypeScript 5.x, `strict: true`, ES2022, ESM-only + `numcodecs` (Blosc, incl. zstd via Blosc; runtime), `@aws-sdk/client-s3` (006-zarr-v3-reader)
- Local filesystem (disk chunk cache), remote stores (FS/HTTP/S3/Reference), optional (006-zarr-v3-reader)
...
## Recent Changes
- 006-zarr-v3-reader: Added TypeScript 5.x, `strict: true`, ES2022, ESM-only + `numcodecs` (Blosc, incl. zstd via Blosc; runtime), `@aws-sdk/client-s3`
