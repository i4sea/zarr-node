...
- **FR-009**: The library MUST correct, on the v3 path, the existing gap where declared v2 filters are parsed but never applied during decode, so the shared decode path applies every declared transform.
